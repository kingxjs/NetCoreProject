...
layui.define(['laytpl', 'laypage', 'layer', 'form'], function (exports) {
    ...
    Class.prototype.pullData = function (curr, loadIndex) {
        ...
        if (!options.treeRootId) {
            options.treeRootId = "0";
        }
        if (options.url) { //Ajax请求
            ...
            $.ajax({
                ...
                , success: function (res) {
                    var data = that.filterArray(res.data, options.treeId, options.treeUpId, options.treeRootId, options.isOpen);
                    res.data = data[0];
            ...
            var data = that.filterArray(options.data, options.treeId, options.treeUpId, options.treeRootId, options.isOpen);
            res.data = data[0];
     ...
     * @param field_RootId    树形结构默认根目录Id
     * @param isOpen    初始是否打开
     * @returns {Array}     [0]表格列表  [1]树形结构
     */
    Class.prototype.filterArray = function (data, field_Id, field_upId, field_RootId, isOpen) {
        var list = [];
        var treeList = [];
        var tableList = [];
        //设置默认参数
        ...
        var that = this
            ...
            , render = function () { //后续性能提升的重点
                ...
                layui.each(data, function (i1, item1) {
                    ...
                    var trs1 = '<tr data-index="' + i1 + '" upids="' + item1["upIds"] + '" data-id="' + item1[options.treeId] + '">' + tds.join('') + '</tr>';

                    if (item1["upIds"]) {
                        if (options.isOpen) {
                            trs1 = '<tr data-index="' + i1 + '" upids="' + item1["upIds"] + '" data-id="' + item1[options.treeId] + '">' + tds.join('') + '</tr>';
                        } else {
                            trs1 = '<tr data-index="' + i1 + '" upids="' + item1["upIds"] + '" data-id="' + item1[options.treeId] + '" style="display: none;">' + tds.join('') + '</tr>';
                        }
                    }
                    trs.push(trs1);
                    var tds_fixed1 = '<tr data-index="' + i1 + '">' + tds_fixed.join('') + '</tr>';
                    if (item1["upIds"]) {
                        if (options.isOpen) {
                            tds_fixed1 = '<tr data-index="' + i1 + '">' + tds_fixed.join('') + '</tr>';
                        } else {
                            tds_fixed1 = '<tr data-index="' + i1 + '"  style="display: none;">' + tds_fixed.join('') + '</tr>';
                        }
                    }
                    trs_fixed.push(tds_fixed1);
    ...
    Class.prototype.events = function () {
        ...
        that.elem.on('click', 'i.layui-tree-head', function () {
            ...
            if (o.isOpen) {//打开状态的，关闭
                var sonO = $("[upids*=_" + o[options.treeId] + "_]");
                sonO.hide();
                sonO.each(function (index) {
                    var aindex = $(this).data('index');
                    var atr = that.layBody.find('tr[data-index="' + aindex + '"]')
                    var o1 = datas[aindex];
                    o1.isOpen = false;
                    if (o1.children.length > 0) {
                        var dbClickI1 = atr.find('.layui-tree-head')
                        if (!o.isOpen) {//打开状态
                            dbClickI1.html('&#xe625;');
                        } else {
                            dbClickI1.html('&#xe623;');
                ...
                var sonO = $("[upids=" + o["upIds"] + "_" + o[options.treeId] + "_]");
                sonO.show();
                sonO.each(function (index) {
                    var aindex = $(this).data('index');
                    var atr = that.layBody.find('tr[data-index="' + aindex + '"]')
        ...
        that.elem.on('click', 'input[name="layTableCheckbox"]+', function () {
            ...
            if (isAll) {
                ...
                //sonO.each(function (index) {
                //    var aindex = $(this).data('index');
                //    var item = $(this).find('input[name="layTableCheckbox"]')[0];
                //    item.checked = checked;
                //    that.setCheckData(aindex, checked);
                //})
                that.syncCheckAll();
            }
