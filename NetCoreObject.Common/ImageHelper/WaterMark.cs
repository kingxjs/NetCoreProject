    ...
    public class WaterMark
    {
        ...
        public static void AddImageSignPic(string imgPath, string filename, string watermarkFilename, int watermarkStatus, int quality, int watermarkTransparency)
        {
            if (!File.Exists(Utils.GetMapPath(imgPath)))
                return;
            byte[] imageBytes = File.ReadAllBytes(Utils.GetMapPath(imgPath));
        ...
        /// <param name="fontname">字体</param>
        /// <param name="fontsize">字体大小</param>
        public static void AddWaterMark(string imgPath, string filename, string watermarkText, int watermarkStatus, int quality, string fontname, int fontsize)
        {
            byte[] imageBytes = File.ReadAllBytes(Utils.GetMapPath(imgPath));
            Image img = Image.FromStream(new System.IO.MemoryStream(imageBytes));
            ...
            g.DrawString(watermarkText, drawFont, semiTransBrush, new PointF(0, 0));
            //g.DrawString(watermarkText, drawFont, new SolidBrush(Color.White), xpos + 1, ypos + 1);
            //g.DrawString(watermarkText, drawFont, new SolidBrush(Color.Black), xpos, ypos);
